...
const activityInfo = {
  ...
  description: "A collaborative speaking activity where classmates describe a sentence word by word, tracking which words they've used.",
  primaryFunction: "Speaking & Circumlocution",
  skills: [
  ...
  howItWorks: [
    ...
    "Classmates describe it, one word at a time",
    "Tap words as they're described to track progress",
    "Guesser reconstructs the full sentence",
  ],
  tips: [
    "Students can describe words in any order",
    "Encourage synonyms and definitions",
    "Track which words are 'used' by tapping them",
    "Award points for fewer words needed to guess",
  ],
};

export default function BackToBoardSentencesSetup() {
  ...
  const [timerMode, setTimerMode] = useState<"per-sentence" | "total" | "off">("per-sentence");
  const [timerSeconds, setTimerSeconds] = useState(120);
  const [perSentenceSeconds, setPerSentenceSeconds] = useState(60);
    ...
    if (typeof window !== 'undefined') {
      ...
      if (stored) {
        try {
          ...
          if (data.perSentenceSeconds) setPerSentenceSeconds(data.perSentenceSeconds);
  ...
  useEffect(() => {
    setSentences((prev) => {
      ...
      while (newSentences.length < numSentences) {
        newSentences.push({ sentence: "", words: [] });
      }
      while (newSentences.length > numSentences) {
        newSentences.pop();
      }
  ...
  const updateSentence = (index: number, value: string) => {
    ...
    newSentences[index] = {
      sentence: value,
      words: value.match(/[\w']+/g) || [],
    };
    setSentences(newSentences);
  };

  const addSentence = () => {
    setSentences([...sentences, { sentence: "", words: [] }]);
  ...
  const generateWithAI = async () => {
    ...
    try {
      ...
      if (data?.sentences) {
        sessionStorage.removeItem("backToBoardSentencesData");
        const newSentences = data.sentences.map((s: { sentence: string }) => ({
          sentence: s.sentence,
          words: s.sentence.match(/[\w']+/g) || [],
        }));
        setSentences(newSentences);
        // Track generated content to avoid repetition
        const newExclusions = data.sentences.map((s: { sentence: string }) => s.sentence);
  ...
  const isValid = sentences.every((s) => s.sentence.trim() && s.words.length > 2);
  ...
  const handleStart = () => {
    ...
    sessionStorage.setItem("backToBoardSentencesData", JSON.stringify(gameData));
    router.push("/activities/back-to-board-sentences/play");
  };
                  ...
                  <ol className="space-y-2">
                    {activityInfo.howItWorks.map((step, i) => (
                      <li key={i} className="flex items-start gap-3 text-sm text-[var(--text-secondary)]">
                        <span className="flex-shrink-0 w-5 h-5 rounded-full bg-amber-500/20 text-amber-500 flex items-center justify-center text-xs font-bold">
                          {i + 1}
                        </span>
                        {step}
                      </li>
          ...
          <div className="max-w-4xl mx-auto flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <button
                ...
                className="p-3 rounded-xl glass-card text-amber-500 hover:text-amber-400 hover:scale-105 transition-all"
              ...
              <div>
                <h1 className="text-xl font-bold text-[var(--text-primary)] flex items-center gap-2">
                  <span>📋</span> Back to Board: Sentences
                </h1>
          ...
          <div className="space-y-6">
                ...
                <div className="glass-card p-5">
                  ...
                  <div className="mb-4">
                    ...
                    <input
                      ...
                      placeholder="e.g., present continuous, conditionals"
                      className="w-full p-4 rounded-xl bg-[var(--glass-bg)] border border-[var(--glass-border)] focus:border-amber-500 text-[var(--text-primary)] placeholder-[var(--text-muted)] outline-none transition-all"
                    />
                  </div>
                  ...
                  <div>
                    ...
                    <input
                      ...
                      placeholder="e.g., daily routines, travel, food"
                      className="w-full p-4 rounded-xl bg-[var(--glass-bg)] border border-[var(--glass-border)] focus:border-amber-500 text-[var(--text-primary)] placeholder-[var(--text-muted)] outline-none transition-all"
                    />
                  </div>
                </div>
            ...
            <div className="glass-card p-5 animate-waterfall animate-waterfall-2">
              ...
              <div className="mb-5">
                ...
                <div className="flex items-center gap-2">
                  <button
                    onClick={() => setNumSentences(Math.max(3, numSentences - 1))}
                    className="w-10 h-10 rounded-lg glass-card text-[var(--text-primary)] font-bold hover:text-amber-500 transition-all"
                  >
                    -
                  </button>
                  <input
                    ...
                    value={numSentences}
                    onChange={(e) =>
                      setNumSentences(Math.max(3, Math.min(10, parseInt(e.target.value) || 3)))
                    }
                    className="flex-1 max-w-[120px] p-3 rounded-lg bg-[var(--glass-bg)] border border-[var(--glass-border)] text-[var(--text-primary)] text-center font-semibold outline-none focus:border-amber-500 transition-all"
                  />
                  <button
              ...
              <div>
                ...
                <div className="flex flex-wrap gap-2 mb-4">
                  {[
                    { id: "off", label: "Off" },
                    { id: "per-sentence", label: "Per Sentence" },
                    { id: "total", label: "Total Time" },
                  ].map((tm) => (
                    <button
                      key={tm.id}
                      onClick={() => setTimerMode(tm.id as "off" | "per-sentence" | "total")}
                      className={`px-4 py-2 rounded-lg font-medium transition-all ${
                        timerMode === tm.id
                ...
                {timerMode !== "off" && (
                  <div className="flex items-center gap-2">
                    <input
                      type="number"
                      value={timerMode === "per-sentence" ? perSentenceSeconds : timerSeconds}
                      onChange={(e) => {
                        const val = Math.max(10, parseInt(e.target.value) || 60);
                        if (timerMode === "per-sentence") {
                          setPerSentenceSeconds(val);
                        } else {
                          setTimerSeconds(val);
                        }
                      }}
                      className="w-24 p-2 rounded-lg bg-[var(--glass-bg)] border border-[var(--glass-border)] text-[var(--text-primary)] text-center outline-none focus:border-amber-500 transition-all text-sm"
                    />
                    <span className="text-[var(--text-muted)] text-sm">
                      seconds {timerMode === "per-sentence" ? "per sentence" : "total"}
                    </span>
                  </div>
                )}
              </div>
            ...
            {mode === "ai" && (
              <button
                ...
                className="w-full p-4 rounded-xl bg-gradient-to-r from-amber-500 to-orange-500 text-white font-bold flex items-center justify-center gap-2 hover:opacity-90 transition-all disabled:opacity-50 shadow-lg shadow-amber-500/25"
              >
                {isGenerating ? (
                  <>
            ...
            <div className="glass-card p-5 animate-waterfall animate-waterfall-3">
                ...
                {sentences.map((item, index) => (
                  ...
                  >
                    ...
                    <input
                      ...
                      className="w-full p-3 rounded-xl bg-[var(--surface)] border border-[var(--glass-border)] focus:border-amber-500 text-[var(--text-primary)] outline-none transition-all placeholder-[var(--text-muted)]"
                    />
                      ...
                      <div className="mt-3">
                        ...
                        <div className="flex flex-wrap gap-2">
                          {item.words.map((word, wordIndex) => (
                            <span
                              key={wordIndex}
                              className="px-2 py-1 rounded-lg bg-amber-500/20 text-amber-600 dark:text-amber-400 text-sm font-medium border border-amber-500/30"
                            >
                              {word}
                            </span>
                          ))}
              ...
              <button
                onClick={addSentence}
                className="w-full mt-4 p-3 rounded-xl border-2 border-dashed border-[var(--glass-border)] hover:border-amber-500/50 hover:bg-amber-500/5 flex items-center justify-center gap-2 text-[var(--text-muted)] hover:text-amber-500 transition-all"
              >
        ...
        <div className="fixed bottom-6 left-6 z-30 animate-waterfall animate-waterfall-4">
          <button
            ...
            className={`px-6 py-4 rounded-2xl font-bold flex items-center gap-3 shadow-2xl transition-all ${
              isValid
                ? "bg-gradient-to-r from-amber-500 to-orange-500 text-white hover:scale-105 hover:shadow-amber-500/30"
